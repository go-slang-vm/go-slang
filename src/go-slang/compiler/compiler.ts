...
import { ASTNode, AssignNode, BinOpNode, BlockNode, ConstDeclNode, ForStmtNode, FuncAppNode, FuncDeclNode, IfStmtNode, LambdaStmtNode, LiteralNode, LogicalNode, NameNode, ReturnStmtNode, SequenceNode, StmtNode, Tag, UnOpNode, VarDeclNode } from "../ast/AST";
...
// of a given symbol x
const compile_time_environment_position = (env: CompileTimeEnvironment, x: string) : FramePosition => {
...
const compile_time_environment_extend = (vs: Frame, e: CompileTimeEnvironment) : CompileTimeEnvironment => {
...
const scan_for_locals = (comp: ASTNode): string[] => {
    ...
    const ret = comp.tag === 'seq'
    ? (comp as SequenceNode).stmts.reduce(
        (acc: string[], x: ASTNode) => 
...
const compile_comp = {
lit:
    (comp: LiteralNode, ce: CompileTimeEnvironment) => {
        instrs[wc++] = { tag: "LDC", 
                         val: comp.val
        };
...
unop:
    (comp: UnOpNode, ce: CompileTimeEnvironment) => {
        compile(comp.frst, ce);
        instrs[wc++] = {tag: 'UNOP', sym: comp.sym};
    },
binop:
    (comp: BinOpNode, ce: CompileTimeEnvironment) => {
        compile(comp.frst, ce);
        compile(comp.scnd, ce);
        instrs[wc++] = {tag: 'BINOP', sym: comp.sym};
    },
log:
    (comp: LogicalNode, ce: CompileTimeEnvironment) => {
...
cond: 
    (comp: IfStmtNode, ce: CompileTimeEnvironment) => {
...
while:
    (comp: ForStmtNode, ce: CompileTimeEnvironment) => {
        const loop_start = wc;
        compile(comp.pred, ce);
        const jump_on_false_instruction: any = {tag: 'JOF'};
        instrs[wc++] = jump_on_false_instruction;
...
app:
    (comp: FuncAppNode, ce: CompileTimeEnvironment) => {
        compile(comp.fun, ce);
...
lam:
    (comp: LambdaStmtNode, ce: CompileTimeEnvironment) => {
        instrs[wc++] = {tag: 'LDF', 
                        arity: comp._arity,
                        addr: wc + 1};
...
seq: 
    (comp: SequenceNode, ce: CompileTimeEnvironment) => compile_sequence(comp.stmts, ce),
blk:
    (comp: BlockNode, ce: CompileTimeEnvironment) => {
...
fun:
    (comp: FuncDeclNode, ce: CompileTimeEnvironment) => {
